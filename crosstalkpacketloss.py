	...
	while(inputVal != "E"):
		...
		if (inputVal == "G"):
			...
			while (att < 12.0):
				...
				Oscope.captureScreenShot("hist_both_att_%f.jpg" %att)
				#For the top Box now
				Oscope.setHistogram("1.01589E-9", "1.02478E-9", "355.16E-3", "222.90E-3")
				Oscope.captureScreenShot("hist_top_att_%f.jpg" %att)
				#For the bottom box now
				Oscope.setHistogram("1.01589E-9", "1.02478E-9", "-212.58E-3", "-351.29E-3")
				Oscope.captureScreenShot("hist_bottm_att_%f.jpg" %att)

				#now to get the qfactor
				Oscope.saveQFactor(att, qfactorFile)
				Oscope.driver._write('display:cgrade off') #turns off color grade
    
                        	# now collect packets for this attn
                        	PCAPReader.RunTrial(100000, 'eth7', directory+"/Attn_"+str(att)+".pcap");

				#insert the bert things here 
				att = att + 1.0 # here is the step 
